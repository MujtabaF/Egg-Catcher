...
c.create_rectangle(-5, canvas_height - 100, canvas_width + 5, \
                   canvas_height + 5, fill='sea green', width=5, outline='green')
c.create_oval(-80, -80, 120, 120, fill='orange', width=0)
c.pack()

#Color Cycle
color_cycle = cycle(['light blue', 'light green', 'light yellow', 'light cyan'])
egg_width = 45
egg_height = 55
egg_score = 10
egg_speed = 500
...
catcher = c.create_arc(catcher_start_x, catcher_start_y, \
                       ...
                       style='arc', outline=catcher_color, width=5)

#Score Counter
game_font = font.nametofont('TkFixedFont')
game_font.config(size=12)
...
def create_egg():
    x = randrange(10, 740)
    y = 40
    new_egg = c.create_oval(x, y, x + egg_width, y + egg_height, fill=next(color_cycle), width=4, outline='black')
    eggs.append(new_egg)
...
#Check if it is caught correctly
def check_catch():
